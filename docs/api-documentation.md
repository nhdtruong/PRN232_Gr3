...
## 8. Bảng điểm (Grades & Transcript)
Giáo viên/Trung tâm nhập điểm và quản lý bảng điểm tổng hợp cho học viên. Tách biệt theo hai luồng: điểm thường xuyên (theo từng buổi) và điểm định kỳ (theo lớp).
- `GET /api/classes/{classId}/gradesheet`: Lấy bảng điểm thường xuyên (DailyAssessment) của cả lớp theo từng buổi học.
- `POST /api/classes/{classId}/gradesheet`: Lưu/Cập nhật điểm thường xuyên (TX) và nhận xét theo từng buổi học. Điểm trung bình TX được tự động tính lại sau khi lưu.
- `GET /api/classes/{classId}/transcript`: Lấy bảng điểm tổng kết (ClassTranscript) — bao gồm Điểm TB Thường xuyên, Giữa kỳ, Cuối kỳ và Tổng kết.
- `POST /api/classes/{classId}/transcript`: Lưu điểm định kỳ (Giữa kỳ, Cuối kỳ) cho cả lớp. Hệ thống tự động tính điểm Tổng kết theo công thức hệ số.
- **Trạng thái chờ phát sóng:** Sau khi giáo viên lưu điểm, dữ liệu ở trạng thái Pending. Phụ huynh chỉ nhận được thông báo điểm khi giáo viên bấm "Phát sóng thông báo" từ màn hình tổng hợp.

## 9. Chức năng Phụ huynh (Parent Features)
...
## 10. Nhắn tin (Chat)
Quản lý kênh giao tiếp giữa Trung tâm với Phụ huynh và Giáo viên.
- `GET /api/Chat/channels`: Lấy danh sách các kênh nhắn tin (Chat rooms) của người dùng hiện tại (áp dụng cho Center, Parent, và Teacher).
...
- **Hộp thư Giáo viên:** Giáo viên (Teacher) trò chuyện trực tiếp với Admin trung tâm thông qua /Teacher/Chat/Index.
- **Bộ lọc Admin:** Danh sách trò chuyện của Admin (Center) tại /Center/Chat/Index gồm 2 tab: Phụ huynh (Parent) và Giáo viên (Teacher - hiển thị đầy đủ thông tin: SĐT, danh sách lớp đang dạy).

## 11. Thông báo (Notifications)
Gửi và hiển thị thông báo thời gian thực (Real-time).

- `GET /api/notifications`: Lấy danh sách thông báo cá nhân của user (hỗ trợ cả Parent và Teacher).
...
- **Phân công giảng dạy:** Khi Giáo viên được gán giảng dạy một lớp mới, hệ thống tự động tạo và gửi thông báo realtime qua SignalR.
- **Thông báo Điểm danh + Điểm số:** Khi giáo viên phát sóng thông báo, Phụ huynh nhận được 1 thông báo tổng hợp duy nhất gồm: trạng thái điểm danh, ghi chú, điểm thường xuyên và nhận xét.
